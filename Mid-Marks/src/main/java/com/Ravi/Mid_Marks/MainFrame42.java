...
public class MainFrame42 extends JFrame implements ActionListener {
	...
	public MainFrame42() {
		...
		model = new DefaultTableModel(new String[] { "S.No", "Roll No", "Name", "1(a)", "1(b)", "2(a)", "2(b)", "3(a)",
				"Assignment", "Quiz", "Total Mid 2" }, 0) {
			@Override
			public boolean isCellEditable(int row, int column) {
				// Make the "Total Mid 2" column (index 10) non-editable
				return column != 10;
			}
		};
		...
		model.addTableModelListener(new TableModelListener() {
			...
			public void tableChanged(TableModelEvent e) {
				tableChanged = true; // Set flag when any changes are made to the table
				updateWindowTitle(); // Update window title to "Not Saved"
			}
		});
		...
		addWindowListener(new WindowAdapter() {
			...
			public void windowClosing(WindowEvent e) {
				handleWindowClosing(); // Custom method to handle closing
			}
		});
	...
	// Method to calculate and update Total Mid 2 for a specific row
	private void calculateAndUpdateTotalMid1ForRow(int row) {
		...
		// Update Total Mid 2 column in the model
	...
	private void exportToExcel() {
		...
		for (int i = 0; i < model.getRowCount(); i++) {
			...
			for (int j = 0; j < model.getColumnCount(); j++) {
				...
				} else if (j >= 3 && j <= 10) { // 1(a), 1(b), 2(a), 2(b), 3, Assignment, Quiz, Total Mid 2
		...
		fileChooser.setSelectedFile(new File("4_2 mid 2.xlsx"));
	...
	private void importDataFromCSV() {
		String csvFile = "resources/3rd year/3rd year Mid 2.csv"; // Update this with your CSV file path
	...
	private void showPieChart() {
		...
		// Create pie chart
		JFreeChart chart = ChartFactory.createPieChart("Student Performance (Total Mid 2)", dataset, true, true, false);
		...
		table.addKeyListener(new java.awt.event.KeyAdapter() {
			public void keyPressed(java.awt.event.KeyEvent e) {
				if (e.getKeyCode() == java.awt.event.KeyEvent.VK_DELETE
					...
					// Ensure the table editor is stopped before modifying the data
					if (table.isEditing()) {
					...
					for (int row : selectedRows) {
						for (int column : selectedColumns) {
							// Skip column 10 (index 10 is non-editable)
							if (column != 10) {
								model.setValueAt(0, row, column); // Set as integer 0
							}
						}
					}

					// Consume the event to prevent default behavior (clearing the cell)
					e.consume();
				}
			}
		});
	...
	private boolean validateMarks() {
		int column8Index = 8;
		int column10Index = 10;

		for (int i = 0; i < model.getRowCount(); i++) {
			// Get the values for column 8 and column 10
			int column9Value = Integer.parseInt(model.getValueAt(i, column8Index).toString());
			int column10Value = Integer.parseInt(model.getValueAt(i, column10Index).toString());

			// Validate column 8 (should not exceed 5)
			if (column9Value > 5) {
				JOptionPane.showMessageDialog(null, "Error: Student in row " + model.getValueAt(i, 1).toString()
						+ " has assignment marks exceeding 5.", "WARNING MESSAGE", JOptionPane.ERROR_MESSAGE);
				return false; // Validation failed
			}

			// Validate column 10 (should not exceed 30)
			if (column10Value > 30) {
				JOptionPane.showMessageDialog(null, "Error: Student in row " + model.getValueAt(i, 1).toString()
						+ " has total marks value exceeding 30.", "WARNING MESSAGE", JOptionPane.ERROR_MESSAGE);
				return false; // Validation failed
			}
		}
	...
	private void handleWindowClosing() {
		if (tableChanged) {
			...
			int option = JOptionPane.showConfirmDialog(this,
					"You have unsaved changes. Do you want to save before exiting?", "Save Changes",
					JOptionPane.YES_NO_OPTION);

			if (option == JOptionPane.YES_OPTION) {
				// Save the changes and then close the application
				updateCSVFile(); // Call method to update CSV file
				dispose(); // Close the GUI
			} else if (option == JOptionPane.NO_OPTION) {
				// Close the application without saving
				dispose(); // Close the GUI
			} // If CANCEL is pressed, do nothing (stay in the GUI)
		} else {
			// No changes were made, just close the application
			dispose(); // Close the GUI
		}
	}
	...
	private void updateWindowTitle() {
		if (tableChanged) {
			setTitle("Student Management - Not Saved");
		} else {
			setTitle("Student Management - Saved");
		}
	}
	...
	private void setupSaveShortcut() {
		...
		actionMap.put("save", new AbstractAction() {
			@Override
			public void actionPerformed(ActionEvent e) {
				updateCSVFile(); // Call the save method
			}
		});
	}

	public static void main(String[] args) {
		SwingUtilities.invokeLater(() -> new MainFrame42().setVisible(true));
	}
}
